...
def build_question_data_v2(
    ...
    # Q11-20: Various formats based on q11_20_type
        ...
        if q11_20_type == "CLOZE_PASSAGE":
            ...
            # Convert to MCQ with AI-generated distractors
            db_question_type = "multiple_choice"
            default_instruction = "Choose the word that means the opposite of the word on the left."
        elif q11_20_type in ["SYNONYM_LETTER", "SYNONYM_SELECTION"]:
            # Convert to MCQ with AI-generated distractors
            db_question_type = "multiple_choice"
            default_instruction = "Choose the word that means the same as the word on the left."
        else:
            db_question_type = "reading_comprehension"
            default_instruction = instruction or "Answer the question."

        # For ANTONYM_LETTER and SYNONYM_LETTER, generate MCQ options with AI distractors
        if q11_20_type in ["ANTONYM_LETTER", "SYNONYM_LETTER"]:
            # Parse given_word and letter_template from q_text
            # q_text format: "enthusiastic -> a_a_h_tic"
            partial_template = None
            if q_text and " -> " in q_text:
                parts = q_text.split(" -> ")
                if len(parts) == 2:
                    given_word = parts[0].strip()
                    partial_template = parts[1].strip()

            # Determine question type for distractor generation
            is_antonym = "ANTONYM" in q11_20_type
            question_type_str = "antonym" if is_antonym else "synonym"

            # Generate MCQ distractors using AI
            try:
                ...
                distractors = generator.generate_distractors(
                    ...
                    test_num=test_num,
                    question_num=q_num,
                    letter_template=partial_template
                )
            ...
            question_data = {
                "question_text": f"Find a word meaning the {'opposite' if is_antonym else 'same'} as: {given_word}" if given_word else q_text,
                ...
                "correct_answer": answer.lower(),
                "case_sensitive": False,
                "given_word": given_word,  # Keep for display
                "letter_template": {       # Keep for display
                    "template": partial_template,
                    "answer": answer.lower()
                } if partial_template else None,
                "answer_options": answer_options
            }
        else:
            # Other Q11-20 types (CLOZE_PASSAGE, ODD_ONE_OUT, etc.)
            question_data = {
